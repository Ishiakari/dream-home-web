    ...
    <div className="bg-white min-h-screen relative pb-20 lg:pb-0">
      ...
      <section className="max-w-[1600px] mx-auto px-4 sm:px-8 py-6">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* LEFT COLUMN: Property List (Slides in from left) */}
          <motion.div 
          ...
          >
            ...
            {[1, 2, 3, 4, 5].map((item, index) => (
              <motion.div
                ...
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, delay: 0.2 + (index * 0.1) }} // Each card delays 0.1s longer than the last
              >
          ...
          {/* RIGHT COLUMN: The Map (Slides in from right) */}
          <motion.div 
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
      ...
      {/* 3. FAQ SECTION (Fades up only when scrolled into view) */}
      <motion.section 
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: "-100px" }} // Triggers just before it hits the screen
        transition={{ duration: 0.6 }}
        className={`max-w-4xl mx-auto px-4 sm:px-8 py-16 border-t border-slate-100 mt-12 ${showMobileMap ? 'hidden lg:block' : 'block'}`}
      >
      ...
      {/* MOBILE FLOATING TOGGLE BUTTON (Spring animation from bottom) */}
      <motion.div 
