...
export default function SearchBar({
  ...
  const [local, setLocal] = useState(
    value ?? { where: "", minPrice: "", maxPrice: "", minRooms: "" }
  );

  // keep local in sync if parent changes (optional but safe)
  useEffect(() => {
  ...
  const parsed = useMemo(() => {
    const minPrice =
      local.minPrice === "" ? null : Number(String(local.minPrice));
    const maxPrice =
      local.maxPrice === "" ? null : Number(String(local.maxPrice));
    const minRooms =
      local.minRooms === "" ? null : Number(String(local.minRooms));
  ...
  const handleSubmit = () => {
    onSearch?.(parsed);
    onChange?.(parsed); // ensure parent has latest even if debounce hasn’t fired
  };
