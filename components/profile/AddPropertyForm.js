...
export default function AddPropertyForm() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setFormData((prev) => ({
                ...prev,
                branch_no: data[0].branch_no || data[0].id,
            }));
            }
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        const submissionData = {
            ...formData,
            branch_no: formData.branch_no, // keep explicit
            no_of_rooms:
            formData.no_of_rooms === "" ? null : parseInt(formData.no_of_rooms, 10),
            monthly_rent:
            formData.monthly_rent === "" ? null : parseFloat(formData.monthly_rent),
        };

        if (!submissionData.branch_no) {
            alert("Please select a managing branch.");
            return;
        }
        ...
        // Reset form on success (keep branch selected if branches exist)
        setFormData({
            title: "",
            description: "",
            street: "",
            ...
            no_of_rooms: "",
            monthly_rent: "",
            branch_no: branches[0]?.branch_no || branches[0]?.id || "",
        });
        } catch (err) {
        console.error("Submit Error:", err);
        console.error("Submit Error details:", err?.details);
    ...
    return (
        ...
        <div className="mb-8 border-b border-slate-100 pb-6">
            ...
            Fill out the details below to add a property to your portfolio.
            </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                <span className="bg-blue-50 text-[#003580] w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold">
                1
                </span>
                Rent &amp; Type
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <input
                    ...
                    required
                    placeholder="e.g. Stunning 2-Bed Flat"
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#003580] outline-none transition text-gray-900 bg-white"
                />
                </div>
                ...
                <select
                    ...
                    onChange={handleChange}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#003580] outline-none transition bg-white text-gray-900"
                >
                ...
                >
                    ...
                    branches.map((b) => (
                        ...
                        {`${b.street || ""}${
                            b.street && b.area ? ", " : ""
                        }${b.area || ""}` || b.city || `Branch ${b.branch_no || b.id}`}
                        </option>
                    ))
                    )}
                </select>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                required
                placeholder="Street"
                className="px-4 py-3 rounded-xl border border-slate-200 text-gray-900 bg-white"
                />
                <input
                type="text"
                name="area"
